...
static void scope_dsp(t_scope *x, t_object *dsp, short *count, double samplerate, long maxvectorsize, long flags)
{
    ...
    {
        x->f_mode = 1;
        //object_method(dsp, gensym("dsp_add"), x, (t_method)scope_perform2, 0, NULL);
        x->f_startclock = 1;
    }
    else if(count[0])
    {
        x->f_mode = 0;
        //object_method(dsp, gensym("dsp_add"), x, (t_method)scope_perform1, 0, NULL);
        x->f_startclock = 1;
    }
}
...
{
    ebox_invalidate_layer((t_ebox *)x, NULL, cream_sym_signal_layer);
...
static void scope_getdrawparams(t_scope *x, t_object *view, t_edrawparams *params)
{
...
static void draw_background(t_scope *x,  t_object *view, t_rect *rect)
{
    ...
    if (g)
    {
        ...
        for(i = 1; i < 4; i++)
        {
            elayer_line_fast(g, -2, rect->height * 0.25 * (float)i, rect->width + 4, rect->height * 0.25 * (float)i);
        }
        for(i = 1; i < 6; i++)
        {
            elayer_line_fast(g, rect->width * (1. / 6.) * (float)i, -2, rect->width * (1. / 6.) * (float)i, rect->height + 4);
        }
...
{
    t_elayer *g = ebox_start_layer((t_ebox *)x, view, cream_sym_signal_layer, rect->width, rect->height);
    if(g)
    {
        float i;
        ...
        elayer_set_color_rgba(g, &x->f_color_signal);
        elayer_set_line_width(g, 2);
        elayer_move_to(g, 0, (x->f_buffer_x[0] - x->f_range[0]) * ratio + rect->height);
        for(i = increment; i < x->f_signal_size; i += increment)
        {
            elayer_line_to(g, i / (float)x->f_signal_size * rect->width, (x->f_buffer_x[(int)i] - x->f_range[0]) * ratio + rect->height);
        }
        
        elayer_stroke(g);
        x->f_index = 0;
        ebox_end_layer((t_ebox *)x, view, cream_sym_signal_layer);
    }
    ebox_paint_layer((t_ebox *)x, view, cream_sym_signal_layer, 0., 0.);
}
...
static void draw_signalXY(t_scope *x, t_object *view, t_rect *rect)
{
    ...
    if (g)
    {
        ...
        elayer_move_to(g, (x->f_buffer_x[0] - x->f_range[0]) * ratiox, (x->f_buffer_y[0] - x->f_range[0]) * ratioy + rect->height);
        for(i = increment; i < x->f_signal_size; i += increment)
        {
            elayer_line_to(g, (x->f_buffer_x[(int)i] - x->f_range[0]) * ratiox, (x->f_buffer_y[(int)i] - x->f_range[0]) * ratioy + rect->height);
        }
...
extern "C" void setup_c0x2escope_tilde(void)
{
    ...
    c = eclass_new("c.scope~", (t_method)scope_new, (t_method)scope_free, (short)sizeof(t_scope), 0L, A_GIMME, 0);
    
    eclass_guiinit(c, 0);
    eclass_dspinit(c);
    eclass_addmethod(c, (t_method) scope_dsp,             "dsp",              A_NULL, 0);
    eclass_addmethod(c, (t_method) scope_paint,           "paint",            A_NULL, 0);
    eclass_addmethod(c, (t_method) scope_getdrawparams,   "getdrawparams",    A_NULL, 0);
    eclass_addmethod(c, (t_method) scope_oksize,          "oksize",           A_NULL, 0);
